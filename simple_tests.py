...
class TestFunctionsLocal(unittest.TestCase):
    def test_get_xml_pattern(self):
        """Check the pattern against hard-coded regex."""
        self.assertEqual(first=get_xml_pattern(tag="tag"), second=r"<tag>(.*?)</tag>")
        with self.assertRaises(expected_exception=ValueError):
    ...
    @patch('alana.prompt.gen_msg')
    @patch('alana.prompt.len', return_value=2)
    def test_gen(self, mock_len, mock_gen_msg):
        """Check that `gen` correctly appends to `messages`."""
        messages: list[MessageParam] = [MessageParam(role="user", content="Hi")]
        output = gen(messages=messages, model="haiku", system="Respond in Spanish", loud=False)
        self.assertEqual(mock_gen_msg.call_count, 1)
        self.assertTrue(output)

    # @patch('alana.prompt.len', return_value=2)
    # # @patch('anthropic.Anthropic')
    # def test_gen_prefill(self,  mock_len):
    #     """Check that `gen` correctly appends to last message of `messages` when the last message is from an assistant."""
        # mock_message = mock_anthropic.messages.create.return_value
        # mock_message = [{'role': 'assistant', 'content': 'Hi there! How can I assist you today?'}]
        # messages: list[MessageParam] = [MessageParam(role="user", content="Hi"),
        #                                 MessageParam(role="assistant", content="Hi")]
        # messages = [{'role': 'assistant', 'content': 'Hi there! How can I assist you today?'}]
        # gen(messages=messages, model="haiku", loud=False, temperature=0.0)
        # # self.assertEqual(first=len(messages), second=2)
        # print(type(messages))
        # print(messages)
        ...
        # self.assertEqual(messages[-1]['content'], 'Hi there! How can I assist you today?')
        # messages = []
        # You can mock the gen_msg function for simplicity in this testcase
    ...
    @patch('anthropic.Anthropic')
    @flaky(max_runs=1, min_passes=1)
    def test_gen_msg_with_default_args(self, mock_anthropic, mock_os):
        """Hard-coded check that `gen_msg` works in a normal use-case"""
        # mock_message = [{'role': 'user', 'content': [{'type': 'text', 'text': 'Hi'}]}, {'role': 'assistant'}]
        mock_os.return_value = 'api_key123'
        mock_model = 'haiku'
        mock_client_instance = mock_anthropic.return_value
        mock_message = mock_client_instance.messages.create.return_value
        mock_message.content[0].text = 'Test response'
        messages: List[MessageParam] = [MessageParam(role="user", content="Test message. Respond with 'Test response'")]
        response: Message = gen_msg(messages=messages, model=mock_model,
                                    temperature=0.0)  # NOTE: This is potential source of flake, bc even temp=0.0 is non-deterministic
        mock_anthropic.assert_called_once_with(api_key="api_key123")
        mock_anthropic.return_value.messages.create.assert_called_with(max_tokens=1024,
                                                                       messages=messages,
                                                                       system="",
                                                                       model=globals.MODELS[mock_model],
                                                                       temperature=0.0)
        self.assertEqual(first=response.content[0].text, second="Test response")
    ...
    #     """Hard-coded check that `gen_msg` reverts to default model when passed in an invalid model."""
    #     messages: List[MessageParam] = [MessageParam(role="user", content="Test message")]
    #     response: Message = gen_msg(messages, model="not_a_model", system="Respond with the phrase `Test response`",
    #                                 temperature=0.0)
    #     self.assertEqual(first=response.content[0].text, second="Test response")
    #     self.assertEqual(first=response.model, second=globals.DEFAULT_MODEL)  # type: ignore
    #     self.assertEqual(len(messages), second=1)
    ...
    #     instruction = "Write a one-sentence story about a magical adventure."
    #     n_examples = 3
    #     examples: List[str] = gen_examples_list(instruction=instruction, n_examples=n_examples, temperature=0.0,
    #                                             model="opus", loud=False)
    #     self.assertEqual(first=len(examples), second=n_examples)
    #     for example in examples:
    #         self.assertIsInstance(obj=example, cls=str)
    #         self.assertGreater(a=len(example), b=1)
    ...
    #     instruction = "Write a two-sentence story about a magical adventure."
    #     n_examples = 3
    #     examples_str: str = gen_examples(instruction, n_examples=n_examples, temperature=0.0, model="sonnet",
    #                                      loud=False)
    #     self.assertIsInstance(obj=examples_str, cls=str)
    #
    #     examples = get_xml(tag="examples", content=examples_str)
